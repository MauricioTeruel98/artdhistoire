    ...
    <div id="chat-window" class="card shadow d-none" style="width: 300px;">
        ...
        <div class="card-body">
            ...
            <div id="chat-messages">
                <p>Bonjour, veuillez laisser vos coordonnées pour pouvoir nous envoyer un message ici.</p>
            </div>
            <form id="contact-form" class="mt-3">
                @csrf
                <div class="mb-3">
                    <input type="text" class="form-control bottom-border" id="name" name="name" placeholder="Nom" required>
                </div>
                <div class="mb-3">
                    <input type="email" class="form-control bottom-border" id="email" name="email" placeholder="Email" required>
                </div>
                <div class="mb-3">
                    <textarea class="form-control bottom-border" id="message" name="message" rows="3" placeholder="Message" required></textarea>
                </div>
    ...
    contactForm.addEventListener('submit', function(e) {
        ...
        if (csrfToken) {
            ...
            .then(data => {
                ...
                if (data.success) {
                    chatMessages.innerHTML += '<p class="text-success">Votre message a été envoyé avec succès!</p>';
                    contactForm.reset();
                } else {
                    chatMessages.innerHTML += '<p class="text-danger">Une erreur s\'est produite. Veuillez réessayer.</p>';
                }
            })
            .catch(error => {
...
<footer class="d-flex flex-column align-items-center justify-content-center my-5">
    <div>
        <div class="d-md-flex gap-5 text-center">
            <p>"Art is with us in order that we may not perish through truth." Friedrich Nietzsche</p>
