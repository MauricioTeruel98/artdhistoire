    ...
    <div id="chat-window" class="card shadow d-none" style="width: 300px;">
        <div class="card-header bg-primary text-white d-flex justify-content-between align-items-center">
            <div class="d-flex align-items-center">
                <img src="{{ asset('path/to/logo.png') }}" alt="Art d'Histoire" class="rounded-circle me-2" width="30" height="30">
        ...
        <div class="card-body">
            ...
            <div id="chat-messages">
                <p>{{ app()->getLocale() == 'fr' ? 'Bonjour, veuillez laisser vos coordonnées pour pouvoir nous envoyer un message ici.' : 'Hello, please leave your contact information to send us a message here.' }}</p>
            </div>
            <form id="contact-form" class="mt-3">
                @csrf
                <div class="mb-3">
                    <input type="text" class="form-control bottom-border" id="name" name="name" placeholder="{{ app()->getLocale() == 'fr' ? 'Nom' : 'Name' }}" required>
                </div>
                <div class="mb-3">
                    <input type="email" class="form-control bottom-border" id="email" name="email" placeholder="Email" required>
                </div>
                <div class="mb-3">
                    <textarea class="form-control bottom-border" id="message" name="message" rows="3" placeholder="{{ app()->getLocale() == 'fr' ? 'Message' : 'Message' }}" required></textarea>
                </div>
...
<footer class="d-flex flex-column align-items-center justify-content-center my-5">
    <div>
        <div class="d-md-flex gap-5 text-center">
            <p>{{ app()->getLocale() == 'fr' ? '"L\'art est avec nous pour que nous ne périssions pas de la vérité." Friedrich Nietzsche' : '"Art is with us in order that we may not perish through truth." Friedrich Nietzsche' }}</p>
