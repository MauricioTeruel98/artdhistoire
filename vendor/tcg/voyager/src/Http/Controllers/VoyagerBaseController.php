...
class VoyagerBaseController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if (strlen($dataType->model_name) != 0) {
            ...
            // Filtrar por posts si el dataType es 'archive'
            if ($dataType->name === 'archives') {
                // Obtener el parámetro opcional 'posts' de la URL
                $postsParam = $request->query('post_id');

                if ($postsParam) {
                    // Filtrar los datos según el parámetro 'posts'
                    $query->where('post_id', '=', $postsParam); // Ajusta 'posts_column' al nombre real de la columna en la tabla
                }
            }

            // Filtrar por posts si el dataType es 'videosonline'
            if ($dataType->name === 'videosonline') {
                // Obtener el parámetro opcional 'posts' de la URL
                $postsParam = $request->query('videoonline_id');

                if ($postsParam) {
                    // Filtrar los datos según el parámetro 'posts'
                    $query->where('videoonline_id', '=', $postsParam); // Ajusta 'posts_column' al nombre real de la columna en la tabla
                }
            }
    ...
    public function show(Request $request, $id)
    {
        ...
        return Voyager::view($view, compact('dataType', 'dataTypeContent', 'isModelTranslatable', 'isSoftDeleted', 'request'));
    }
    ...
    public function edit(Request $request, $id)
    {
        ...
        return Voyager::view($view, compact('dataType', 'dataTypeContent', 'isModelTranslatable', 'request'));
    }
    ...
    public function store(Request $request)
    {
        ...
        /*
        if ($dataType->name == 'posts') {
            // Guardar archivos relacionados
            if ($request->has('archives')) {
                foreach ($request->archives as $archive) {
                    // Guardar el archivo y su icono
                    $filePath = $archive['file']->store('archives');
                    $iconPath = $archive['icon']->store('icons');

                    $archiveModel = Archive::create([
                        'post_id' => $data->id,
                        'route' => $filePath,
                        'type' => $archive['type'],
                    ]);
