...
@section('content')
    <div class="page-content">
        ...
        <div class="analytics-container">
            ...
            <div class="Dashboard Dashboard--full" id="sales-dashboard">
                <header class="Dashboard-header">
                    <ul class="FlexGrid">
                        <li class="FlexGrid-item">
                            <div class="Titles">
                                <h1 class="Titles-main">{{ __('Statistiques des Ventes') }}</h1>
                                <div class="Titles-sub">
                                    <form id="month-selector-form" style="display: inline-block;">
                                        <select name="month" id="month-selector" class="form-control" style="width: 200px;">
                ...
                <ul class="FlexGrid FlexGrid--halves">
                    <li class="FlexGrid-item">
                        <div class="Chartjs" style="height: 400px;">
                            <header class="Titles">
                                <h1 class="Titles-main">{{ __('Ventes en USD') }}</h1>
                                <div class="Titles-sub">{{ __('30 derniers jours') }}</div>
                            </header>
                            <div style="position: relative; height: 300px;">
                                <canvas id="sales-usd-container"></canvas>
                            </div>
                        </div>
                    </li>
                    <li class="FlexGrid-item">
                        <div class="Chartjs" style="height: 400px;">
                            <header class="Titles">
                                <h1 class="Titles-main">{{ __('Ventes en EUR') }}</h1>
                                <div class="Titles-sub">{{ __('30 derniers jours') }}</div>
                            </header>
                            <div style="position: relative; height: 300px;">
                                <canvas id="sales-eur-container"></canvas>
                            </div>
                        </div>
                    </li>
                </ul>
            ...
            {{-- Google Analytics Container (existing code) --}}
...
@section('javascript')
    ...
    <script>
        // Datos de ventas
        const salesUSD = @json($salesUSD);
        const salesEUR = @json($salesEUR);

        console.log('Datos USD:', salesUSD);
        console.log('Datos EUR:', salesEUR);
        ...
        // Función para obtener el valor de venta para una fecha específica
        function getSaleValueForDate(sales, date) {
            const sale = sales.find(s => s.date === date);
            return sale ? parseFloat(sale.total) : 0;
        }

        // Agregar el evento change al selector de mes
        document.getElementById('month-selector').addEventListener('change', function() {
            document.getElementById('month-selector-form').submit();
        });

        // Modificar la función getDaysInMonth
        function getDaysInMonth(yearMonth) {
            const date = moment(yearMonth, 'YYYY-MM');
            const now = moment();
            const isCurrentMonth = date.format('YYYY-MM') === now.format('YYYY-MM');
            
            // Si es el mes actual, usar el día actual como límite
            const lastDay = isCurrentMonth ? now.date() : date.daysInMonth();
            const days = [];
            
            for (let i = 1; i <= lastDay; i++) {
                const currentDate = moment(yearMonth, 'YYYY-MM').date(i).format('YYYY-MM-DD');
        ...
        // Obtener array de fechas para el mes seleccionado
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Configurar gráfica USD
            const ctxUSD = document.getElementById('sales-usd-container');
            if (ctxUSD) {
                new Chart(ctxUSD, {
                    ...commonOptions,
                    data: {
                        ...
                        datasets: [{
                            label: 'Ventes USD',
                            data: dates.map(date => getSaleValueForDate(salesUSD, date)),
            ...
            // Configurar gráfica EUR
            const ctxEUR = document.getElementById('sales-eur-container');
            if (ctxEUR) {
                new Chart(ctxEUR, {
                    ...commonOptions,
                    data: {
                        ...
                        datasets: [{
                            label: 'Ventes EUR',
                            data: dates.map(date => getSaleValueForDate(salesEUR, date)),
